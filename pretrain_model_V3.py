...
class GNN(nn.Module):
    ...
    def forward(self, data):
        ...
        _, _, loss_rout = self.BayesTreeVQ(projected)
...
class MultiModalTreeVQ(nn.Module):
    ...
    """
    def __init__(self, *, num_modalities: int = 4, depth: int = 6, latent_dim: int = 32,
    ...
    def forward(self, latents_in: List) -> Tuple[List[List[torch.Tensor]],
        ...
        align_loss = hierarchical_alignment_loss(all_idx, all_z,
                                                 ...
                                                 gamma=self.gamma,
                                                 lambd=self.lambd)
        total_loss = vq_loss_total + self.beta*kl * self._scale + align_loss * self._scale
        return all_idx, all_z, total_loss
    
    def set_warmup_scale(self, s: float):
        self._scale = float(s)