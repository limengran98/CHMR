...
def eval_func(pred, true, reduction=True):
    ...
    if task_type == 'classification':
        ...
        if true.shape[1] == 1:
            rmse_list = []
            is_labeled = ~np.isnan(true)
            rmse_score = np.sqrt(mean_squared_error(true[is_labeled], pred[is_labeled]))
            rmse_list.append(rmse_score)
            return {"avg_mae": np.mean(rmse_list), "mae_list": rmse_list}
        else:
            mae_list = []
            for i in range(true.shape[1]):
...
def save_prediction(model, device, loader, dataset, output_dir, seed):
    ...
    data_name = "_".join(dataset.name.split("-")) if "mol" in dataset.name else dataset.name
    assay_path = f"raw_data/{data_name}/raw/assays.csv.gz"
    assay_df = pd.read_csv(assay_path, compression="gzip")
    if "mol" in dataset.name:
